...
function UserTypePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setShowModal(false);
      fetchUserTypes();
      showToast.success(`Sucessfully ${editUserType ? 'Edited' : 'Created'} ${form.userType_name}`)
    } catch (err) {
      showToast.error(`Failed to ${editUserType ? 'Edit' : 'Create'}  ${form.userType_name}`)
  ...
  const toggleStatus = async (id, action) => {
    try {
      ...
      fetchUserTypes();
      showToast.success(`Sucessfully ${action}d ${form.userType_name}`)
    } catch (err) {
      showToast.error(`Failed to ${action} ${form.userType_name}`)
