...
function SalesPage() {
  ...
  const fetchOptions = async () => {
    ...
    const endpoints = {
      customer: `${Api}/master/view_activeCustomer/bysalesman/${cid}`,
...
const handleSave = async () => {
  ...
  try {
    ...
    formreset();
    fetchSales();
    showToast.success(`Successfully ${editSales ? 'Edited' : 'Created'} Sale.`);
  } catch (err) {
...
{selectedPaymentMethod && (
  <Row className="mb-3">
    <Col md={6}>
      ...
      <Form.Control
        type="number"
        value={form[`expected_${selectedPaymentMethod}`] || ''}
        onChange={(e) =>
          setForm({ ...form, [`expected_${selectedPaymentMethod}`]: e.target.value })
        }
      />
    </Col>
    <Col md={6}>
